...
int main() {
	...
	Shader SpecularShaderProgram("./shaders/vertex.glsl", "./shaders/specular-fragment.glsl", "./shaders/explode-geometry.glsl");
	...
	std::cout << "\nBasic: " << std::endl;
	Model Basic(modelName, Textures);
	glm::vec3 BasicPosition = glm::vec3(-3.0f, 0.0f, 0.0f);
	...
	std::cout << "\nSpecular: " << std::endl;
	Model Specular(modelName, Textures);
	glm::vec3 SpecularPosition = glm::vec3(0.0f, 0.0f, 0.0f);
	glm::mat4 Specular_model = glm::mat4(1.0f);
	Specular_model = glm::translate(Specular_model, SpecularPosition);
	Specular_model = glm::scale(Specular_model, glm::vec3(1.0f, 1.0f, 1.0f) * 0.7f);
	Specular.model = Specular_model;
	Specular.UpdateLight(SpecularShaderProgram , lightColor, lightPosition);

	std::cout << "\nNormal: " << std::endl;
	Model Normal(modelName, Textures);
	glm::vec3 NormalPosition = glm::vec3(3.0f, 0.0f, 0.0f);
	glm::mat4 Normal_model = glm::mat4(1.0f);
	Normal_model = glm::translate(Normal_model, NormalPosition);
    ...
    while (!glfwWindowShouldClose(window)) {
		...
		Specular.UpdateCamera(SpecularShaderProgram, camera);
		Specular.UpdateLight(SpecularShaderProgram, lightColor, lightPosition);
		...
		Specular.Draw(SpecularShaderProgram, camera);
